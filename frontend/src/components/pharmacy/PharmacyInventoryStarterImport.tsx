...
export function PharmacyInventoryStarterImport({
  ...
  const selectFile = (file: File | undefined) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      toast({
        ...
        description: "Upload an Excel file no larger than 5 MB.",
      });
      return;
    }
    if (file.size === 0) {
      toast({
        variant: "destructive",
        title: "Empty file",
        description: "Choose an Excel file containing inventory rows.",
      });
      return;
    }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      toast({
        variant: response.skipped > 0 ? "destructive" : "default",
        title:
          response.created + response.updated === 0
            ? "No inventory rows imported"
            : response.skipped > 0
              ? "Inventory imported with errors"
  ...
  if (compact) {
    return (
      <Card
        className={`overflow-hidden rounded-[8px] border-slate-200 bg-white shadow-sm ${className}`}
      >
          ...
          >
            ...
            {selectedFile ? (
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-sm text-slate-500">
                  .xlsx or .xls, up to 5 MB and 1,000 rows
                </p>
              </div>
            )}
          </div>
  ...
  return (
    <Card
      className={`overflow-hidden rounded-[8px] border-slate-200 bg-white shadow-sm transition-shadow duration-700 hover:shadow-xl ${className}`}
    >
        ...
        >
          ...
          {selectedFile ? (
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-500">
                Supports .xlsx and .xls files up to 5 MB and 1,000 rows
              </p>
            </div>
          )}
        </div>
...
function ImportErrors({ summary }: { summary: InventoryImportSummary }) {
  ...
  return (
    ...
    >
      ...
      <div className="max-h-48 space-y-1 overflow-y-auto break-words">
